...
// Counts per category, computed once — totals over the whole feed that never
// change with the active filter.
const COUNTS = CATEGORY_ORDER.reduce<Record<string, number>>((acc, id) => {
  acc[id] = CHANGELOG.filter((e) => e.category === id).length;
  return acc;
}, {});
