...
const BRANCHES = [
  { name: "main", status: "primary", color: "text-prism-cyan-600", glow: "var(--color-prism-cyan-100)" },
  { name: "pr-214", status: "preview", color: "text-prism-yellow-600", glow: "var(--color-prism-yellow-100)" },
  { name: "feat-auth", status: "booted 7ms", color: "text-prism-red-500", glow: "var(--color-prism-red-100)" },
]
      ...
      <div className="relative flex w-full flex-col overflow-hidden rounded-lg border border-border bg-card shadow-[0_8px_24px_-12px_rgba(21,21,21,0.16)]">
        <div className="relative flex items-center justify-between px-4 pt-3 font-mono text-[0.625rem] uppercase tracking-[0.08em] text-black">
          ...
          <span className="flex items-center gap-1.5">
            <motion.span
              ...
              initial={false}
              animate={run ? { opacity: [1, 0.35, 1], scale: [1, 0.8, 1] } : { opacity: 1, scale: 1 }}
        ...
        <div className="relative flex flex-1 items-center justify-center py-2">
          ...
          <motion.img
            ...
            transition={run ? { duration: 4.6, repeat: Infinity, ease: "easeInOut" } : { duration: 0 }}
          />
        </div>
