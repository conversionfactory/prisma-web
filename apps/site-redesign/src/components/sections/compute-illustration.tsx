...
function Bar({ className }: { className?: string }) {
  return (
    <span className={cn("relative inline-block h-1.5 overflow-hidden rounded-full bg-foreground/10", className)}>
      <span
        aria-hidden
...
const SPARKLES = [
  ...
  { top: "16%", left: "90%", size: 10, color: "var(--color-prism-cyan-400)", duration: 1.9, delay: 0.5 },
  { top: "54%", left: "97%", size: 12, color: "#ffffff", duration: 1.4, delay: 1.1 },
  { top: "82%", left: "72%", size: 9, color: "var(--color-prism-yellow-300)", duration: 2.1, delay: 0.3 },
  { top: "42%", left: "44%", size: 8, color: "#ffffff", duration: 1.7, delay: 0.9 },
  { top: "6%", left: "82%", size: 8, color: "var(--color-prism-red-400)", duration: 1.5, delay: 1.5 },
      ...
      <div className="relative flex w-full flex-col overflow-hidden rounded-lg border border-border bg-card shadow-[0_8px_24px_-12px_rgba(21,21,21,0.16)]">
        ...
        >
          <motion.div
            ...
            initial={false}
            animate={run ? { opacity: [0.3, 0.7, 0.3], scale: [0.92, 1.05, 0.92] } : { opacity: reduce ? 0.4 : 0 }}
            transition={run ? { duration: 3.2, repeat: Infinity, ease: "easeInOut" } : { duration: 0 }}
          />
          ...
          {!reduce &&
            SPARKLES.map((s, i) => (
              <motion.span
                ...
                initial={false}
                animate={run ? { scale: [0, 1, 0], opacity: [0, 1, 0], rotate: [0, 90] } : { scale: 0, opacity: 0 }}
                transition={
                  run
                    ? { duration: s.duration, delay: s.delay, repeat: Infinity, repeatDelay: 0.7, ease: "easeInOut" }
                    : { duration: 0 }
                }
              >
                <Sparkle
